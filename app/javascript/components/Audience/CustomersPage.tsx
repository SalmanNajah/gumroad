      ...
      <section>
        ...
        <CardContent asChild>
          <section>
            {options.length > 0 ? (
              isEditing ? (
                <FormFieldset state={selectedOptionId.error ? "danger" : undefined} className="grow basis-0">
                  <FormSelect
          ...
          <CardContent asChild>
            <FormFieldset>
              <NumberInput value={seats} onChange={(seats) => setSeats(seats ?? 0)}>
                {(props) => <FormInput type="number" {...props} min={1} aria-label="Seats" className="grow" />}
              </NumberInput>
              <div
                style={{
                  width: "100%",
...
const RefundForm = ({
  ...
  return (
    <>
      <FormFieldset state={refundAmountCents.error ? "danger" : undefined} className={className}>
        <PriceInput
